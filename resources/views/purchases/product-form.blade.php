<div class="col-12 product-row border border-top-0 border-left-0 border-right-0 pt-3" data-index="{{$index}}">
    <div class="row">
                ...
                <div class="col-sm-9">
                    <select class="form-control unique select2_select select2-hidden-accessible product_id" name="product_id[{{$index}}]" id="product_id_{{$index}}" data-placeholder="Select Product">
                        ...
                        @foreach($products as $product)
                            <option value="{{$product['id']}}" {{isset($prod) ? ($prod['product_id'] == $product['id'] ? 'selected' : '') : (old('product_id.'.$index) == $product['id'] ? 'selected' : '')}}>
        ...
        <div class="col-md-2">
            <a href="#" class="btn btn-danger remove-product mr-3" data-index="{{$index}}" data-toggle="tooltip" data-original-title="Remove Product"><i class="fa fa-minus font-14 mr-md-3"></i> Hapus Barang</a>
        </div>
        <div class="col-md-5">
        ...
        <div class="col-md-5">
            <div class="form-group required row {{\App\Utils\AppUtils::formGroupError($errors, 'sub_total.'.$index)}}">
                ...
                <div class="col-sm-9">
                    <input class="form-control {{\App\Utils\AppUtils::inputFieldError($errors, 'sub_total.'.$index)}} sub_total" name="sub_total[{{$index}}]" value="{{isset($prod) ? $prod['sub_total'] : old('sub_total.'.$index)}}" id="sub_total_{{$index}}" type="number" placeholder="Sub Total" readonly>
