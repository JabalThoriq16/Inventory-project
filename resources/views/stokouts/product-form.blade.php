<div class="col-12 product-row border border-top-0 border-left-0 border-right-0 pt-3" data-index="{{$index}}">
    <div class="row">
        ...
        <div class="col-md-2">
            <a href="#" class="btn btn-danger remove-product mr-3" data-index="{{$index}}" data-toggle="tooltip" data-original-title="Remove Product"><i class="fa fa-minus font-14 mr-3"></i> Hapus Barang</a>
        </div>
        <div class="col-md-5">
            <div class="form-group required row {{\App\Utils\AppUtils::formGroupError($errors, 'unit_price.'.$index)}}">
                ...
                <div class="col-sm-9">
                    <input class="form-control {{\App\Utils\AppUtils::inputFieldError($errors, 'unit_price.'.$index)}} unit_price" name="unit_price[{{$index}}]" value="{{isset($prod) ? $prod['unit_price'] : old('unit_price.'.$index)}}" id="unit_price_{{$index}}" type="hidden" placeholder="Unit Price">
        ...
        <div class="col-md-5">
            <div class="form-group required row {{\App\Utils\AppUtils::formGroupError($errors, 'sub_total.'.$index)}}">
                ...
                <div class="col-sm-9">
                    <input class="form-control {{\App\Utils\AppUtils::inputFieldError($errors, 'sub_total.'.$index)}} sub_total" name="sub_total[{{$index}}]" value="{{isset($prod) ? $prod['sub_total'] : old('sub_total.'.$index)}}" id="sub_total_{{$index}}" type="hidden" placeholder="Sub Total">
