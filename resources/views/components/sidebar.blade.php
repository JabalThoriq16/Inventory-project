    ...
    <div id="sidebar-collapse">
        <div class="admin-block d-flex">
            ...
            <div class="admin-info">
                <div class="font-strong">Admin</div><small class="text-capitalize">admin</small></div>
        </div>
        <ul class="side-menu metismenu">
            ...
            @endcanany
            @canany(['create stokouts', 'read stokouts'])
                <li class="{{$activeNav == 'stokouts' ? 'active' : ''}}">
                    <a href="javascript:;"><i class="sidebar-item-icon fa fa-truck"></i>
                        <span class="nav-label">Stokouts</span><i class="fa fa-angle-left arrow"></i></a>
                    <ul class="nav-2-level collapse">
                        @can('create stokouts')
                            <li>
                                <a class="{{url()->current() == route('stokouts.create') ? 'active' : ''}}" href="{{route('stokouts.create')}}">Create New</a>
                            </li>
                        @endcan
                        @can('read stokouts')
                            <li>
                                <a class="{{url()->current() == route('stokouts.index') ? 'active' : ''}}" href="{{route('stokouts.index')}}">Manage Purchases</a>
                            </li>
                        @endcan
                    </ul>
                </li>
            @endcanany
            <li>
                <a class="{{url()->current() == route('opname.index') ? 'active' : ''}}" href="{{route('opname.index')}}"><i class="sidebar-item-icon fa fa-calendar"></i>
